...
namespace SegmentationCreator.Process
{
    public static class PLeaves
    {
        ...
        static public ProcessData Run(string path, bool AFF = false)
        {
            ...
            if (SAVE) im.Convert<Bgr, byte>().Save(@"E:\Biostimulant\origin.bmp");
            ...
            for (int i = 0; i < im.Rows; i++)
            {
                for (int j = 0; j < im.Cols; j++)
                {
                    ...
                    if (im[i, j].Hue > 30 && im[i, j].Hue < 90 && im[i, j].Value > 8 && im[i, j].Satuation > 140)
                    {
