...
namespace SegmentationCreator.Process
{
    static class ZhangSuen
    {
        ...
        public static Image<Gray, Byte> Skeleton(Image<Gray, Byte> im)
        {
            /*Image<Gray, float> imF = im.Copy().Convert<Gray,float>();
            imF._Mul(1 / 255.0);
            */

            byte[,,] mat = im.Copy().Data;
            ZhangSuenThinning(mat);

            Image<Gray, Byte> res = new Image<Gray, byte>(mat);
            return res;
        }
