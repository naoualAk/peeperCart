...
namespace pepperSoft.Modele.AlliedVision
{
    ...
    public class VimbaHelper
    {
                ...
                foreach (string supportedPixelFormat in supportedPixelFormats)
                {
                    foreach (string availablePixelFormat in availablePixelFormats)
                    {
                            ...
                            && (pixelFormatFeature.IsEnumValueAvailable(supportedPixelFormat) == true))
                        {
                            ...
                            pixelFormatFeature.EnumValue = "BGR8Packed";
                            pixelFormatSet = true;
                            break;
                        }
                    }
        ...
        public void LoadCameraSettings(string cfgPath)
        {

            if (m_Camera == null || cfgPath == "")
                return;

            try
            {
                m_Camera.LoadCameraSettings(cfgPath);
            }
            catch
            {
                throw new Exception("Loading of camera settings is failed");
            }
        }
